...
//        System.out.println(jdbc.generateId("accounts","acc_no",con));
        for(int i=CurrentId;i<CurrentId+1;i++){
            ...
            int val=jdbc.select("select * from accounts where acc_no="+i);
            if(val==1){
                System.out.println("Successfully created new account for "+name);
            }
        }
